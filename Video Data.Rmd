...
rao.spacing.test(circular(mort_circ$theta[mort_circ$camera.x == "4"]))
...
rtv <- realtime_events %>%
  ...
  #group_by(camera.x) %>%
  # create a column with the most recent feed before each event
  mutate(last_feed_time = lag(ifelse(event_type == "Moult", event_true_datetime, NA))) %>%
...
ggplot(rtv, aes(x = since_feed)) +
  ...
  labs(
    x = "Time since previous moult (hours)",
    y = "Count of mortalities",
