...
ggplot km curve, super ugly
```{r}
#generate legend for prev graph
legend("center", legend = names(km_fit$strata),
       col = 1:5, lwd = 2)  

# Tidy survival object into a data frame
km_fit_clean <- broom::tidy(km_fit)

# Plot
ggplot(km_fit_clean, aes(x = time, y = estimate, color = strata)) +
  geom_step(size = 1.2) +
...
```
#Chattage better version of hazard raio from above with ggplot
```{r}
library(ggplot2)
