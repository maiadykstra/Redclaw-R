...
```{r Adding to treatments_morts_dets}
treatments_morts_dets <- treatments_morts_dets %>%
...
```{r Carapace length wrangling and tests}
...
```{r Summary tables and tests for moulting vs. mortality correlation}
...
summary_simple <- events_summary %>%
  filter(!event_type %in% c("Feed", "Test")) %>%
  complete(event_true_date, event_type, fill = list(count = 0)) %>%
group_by(event_true_date, event_type) %>%
  summarise(
    count = sum(count),
    .groups = "drop"
  )

summary_wide <- summary_simple%>%
  pivot_wider(
...
ggplot(summary_simple, aes(x = event_true_date, y = count, colour = event_type)) +
...
cor.test(
  summary_wide$Moult,
  summary_wide$Mortality,
  method = "kendall"
)
```

Making appropriate data frame for survival curve
```{r Making dataframes for surv curves}
...
```{r Base cox models (assumptions not met)}
...
```{r Linear vs. log time for TV cox}
...
## Binary Logistic Regressions (stats and vis) ##

Moult stage x cannibalism binary logistic regression
```{r Cannibalism by Moult Stage}
# Binary encode cause
...
ggplot() +
  ...
  geom_ribbon(data = newdat, aes(x = Day, y = fit, color = time, fill = time, ymin = lwr, ymax = upr), 
              alpha = 0.2, color = NA) +
  labs(
  ...
  scale_color_manual(
    values = c("Day" = "#ff7f0e", "Night" = "#1f77b4"),  # keep or adjust colors
    labels = c("Day" = "Photophase", "Night" = "Scotophase")
  ) +
  scale_fill_manual(
    values = c("Day" = "#ff7f0e", "Night" = "#1f77b4"),  # same colors as above
    labels = c("Day" = "Photophase", "Night" = "Scotophase")
  ) +
  theme_pubr()
...
Binary logistic regression by mortality cause
```{r Mortality by cause}
...
```{r Circular event occurance graphing}
...
## Simple stats (fisher, chi, etc) ##

Data formatting
```{r Data wrangling and simple stats}
...
```{r Preliminary stats for presentation}
...
### initial data visualization (not used) ###
...
mutate(since_feed = as.numeric(difftime(event_true_datetime, last_feed_time, units = "hours"))) %>%
  ...
  filter (time_of_day == "pm") %>% #edit line for time period of interest or comment out
  ungroup()

#ks.test(rtv$since_feed, "punif", min = 0, max = 12)
#ks.test (rtv$num_time_only, "punif", min = 0, max = 24)

# To make 24 hour graph: change time_since_feed to num_time_only, comment out 
#everything after the creation of num_time_only above (except moult filter)
ggplot(rtv, aes(x = since_feed)) +
  geom_histogram(binwidth = 1, boundary = 0, fill = "steelblue", color = "black") +
  labs(
    x = "Time (Hours, Scotophase)",
    y = "Number of Moults"
  ) +
...
```{r Bar plot Mortality by treatment}
...
```{r Bar plot isolated mort causes}
...
```{r Bar co-reared condensed mort causes %}
...
```{r Bar condensed co-reared mort causes raw}
...
```{r Bar isolated mort causes raw}
treatments_morts_simplify %>%
  filter(treatment == "individual") %>%
    group_by(Cause) %>%
  summarise(count = n(), .groups = "drop") %>%
...
```{r Stacked bar both treatments mort causes}
...
```{r Bar cannibalism by moult stage}
...
boxplot of above
```{r Box cannibalism by moult stage}
treatments_morts_simplify %>%
  filter(treatment == "group") %>%
  filter(Cause != "Moult Failure") %>%
  mutate(Stage = factor(Stage, levels = c("Pre-moult", "Moulting", "Post-moult", "Inter-moult"))) %>%
...
```{r Box cannibalism by calcification}
treatments_morts_simplify %>%
  filter(treatment == "group") %>%
  filter(Cause != "Moult Failure") %>%
  mutate(Stage = factor(Stage, levels = c("Pre-moult", "Moulting", "Post-moult", "Inter-moult"))) %>%
...
```{r Stacked bar cannibalism by moult stage over time}
