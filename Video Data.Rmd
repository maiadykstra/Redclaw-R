...
run_chi_or_fisher <- function(tab) {
  # tab = contingency table (matrix or from xtabs)
  
  # Run chi-square without printing immediately
  chi <- suppressWarnings(chisq.test(tab))
  
  # Check smallest expected count
  min_expected <- min(chi$expected)
  
  if (min_expected < 5) {
    message("Some expected counts < 5. Using Fisher’s Exact Test instead.")
    return(fisher.test(tab))
  } else {
    message("All expected counts >= 5. Using Chi-square Test.")
    return(chi)
  }
}
