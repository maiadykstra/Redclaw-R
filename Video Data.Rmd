...
Moult stage x cannibalism
```{r}
# Binary encode cause
cann_by_moult <- treatments_morts_dets %>%
  filter(treatment == "group") %>% 
  filter(status == 1) %>%  
  filter(Cause != "Moult Failure") %>% # only dead individuals
  mutate(moult_bin = ifelse(Stage == "Post-moult", 1, ifelse(Stage == "Moulting", 1, 0)))


# Fit logistic regression
model <- glm(moult_bin ~ Day, data = cann_by_moult, family = binomial)

summary(model)

# Need to fix formatting on this to make it less ugly, add theme created for presentaiton
...
wilcox.test(ctable_tank_cause)

fisher_cause <- fisher.test(ctable_group_cause)
fisher_cause
